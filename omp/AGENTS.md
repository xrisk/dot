...
- Auto-collab runtime flow is documented in `auto-collab-extension/AGENTS.md`: OMP session start opens a quiet host websocket, registers a dashboard record with a bounded conversation summary at `https://omp.rishav.io/api/sessions`, refreshes it, relays encrypted guest frames, and deletes the record on shutdown.
